...
from models.mamba.rpe_mamba import RPEMambaLayer, PEMambaLayer
from models.transformer.vanilla_transformer import TransformerLayer
from models.transformer.flash_atten_transformer import FlashTransformerLayer
...
class SpotGuidedFlashGeoTransformer(nn.Module):
    def __init__(self, cfg):
        super(SpotGuidedFlashGeoTransformer, self).__init__()
        self.k = cfg.k            # num of neighbor points whose corresponding patches are candidate spots
        self.spots = cfg.spots    # num of neighbor points whose corresponding patches are selected as spots
        self.down_k = cfg.down_k  # num of nodes for down sampling and fusion with semi-dense points
        self.spot_k = cfg.spot_k  # num of points in a spot
        ...
        self.seed_threshold = cfg.seed_threshold
        self.dual_normalization = cfg.dual_normalization
        self.mamba_layers_orders=cfg.mamba_layers_orders
        ...
        self.order_indicator = nn.Sequential(
            nn.Linear(128, 256),
            nn.GELU(),
            nn.Linear(256, 128)
        )
        self.serialization_func=serialization_func

        for _ in range(self.blocks):
            self.upsampling.append(Upsampling(cfg.hidden_dim, cfg.hidden_dim))
            self.downsampling.append(Downsampling(cfg.hidden_dim, cfg.hidden_dim))
            if self.geometric_structure_embedding:
                self.self_attentions.append(FlashTransformerLayer(
                    cfg.hidden_dim, cfg.num_heads, cfg.dropout, cfg.activation_fn
                ))
            else:
                self.self_attentions.append(FlashTransformerLayer(
    ...
    def forward(self,ref_points,src_points, ref_feats,src_feats, ref_points_c,src_points_c, ref_feats_c,src_feats_c):
        ...
        for i in range(self.blocks):
            ...
            ref_points_ct=ref_points_c
            src_points_ct=src_points_c
            if self.geometric_structure_embedding:
                #加入序列化点云
                ref_embeddings_t=ref_embeddings
                src_embeddings_t=src_embeddings
                ref_points_ct,ref_feats_c, ref_embeddings_t, inverse_order_r, bs0, n_p0 = self.serialization_func(ref_points_ct,ref_feats_c, ref_embeddings_t, self.mamba_layers_orders[i])
                src_points_ct,src_feats_c, src_embeddings_t, inverse_order_s, bs1, n_p1 = self.serialization_func(src_points_ct,src_feats_c, src_embeddings_t, self.mamba_layers_orders[i])
                ref_feats_c,_ = self.self_attentions[i](ref_feats_c, ref_feats_c, ref_embeddings_t)
                src_feats_c,_ = self.self_attentions[i](src_feats_c, src_feats_c, src_embeddings_t)
                ref_feats_c = ref_feats_c.flatten(0, 1)[inverse_order_r].reshape(bs0, n_p0, -1).contiguous()
                #ref_feats_c=self.order_indicator(ref_feats_c)

                src_feats_c = src_feats_c.flatten(0, 1)[inverse_order_s].reshape(bs1, n_p1, -1).contiguous()
                #src_feats_c=self.order_indicator(src_feats_c)
            else:
                ref_points_ct,ref_feats_c, inverse_order_r, bs0, n_p0 = self.serialization_func(ref_points_ct,ref_feats_c, None, self.mamba_layers_orders[i])
                src_points_ct,src_feats_c, inverse_order_s, bs1, n_p1 = self.serialization_func(src_points_ct,src_feats_c, None, self.mamba_layers_orders[i])
                ref_feats_c,_ = self.self_attentions[i](ref_feats_c, ref_feats_c, ref_embeddings_t)
                src_feats_c,_ = self.self_attentions[i](src_feats_c, src_feats_c, src_embeddings_t)
                ref_feats_c = ref_feats_c.flatten(0, 1)[inverse_order_r].reshape(bs0, n_p0, -1).contiguous()
                #ref_feats_c=self.order_indicator(ref_feats_c)
...
class SpotGuidedGeoMamba(nn.Module):
    def __init__(self, cfg):
        super(SpotGuidedGeoMamba, self).__init__()
        self.k = cfg.k            # num of neighbor points whose corresponding patches are candidate spots
        self.spots = cfg.spots    # num of neighbor points whose corresponding patches are selected as spots
        self.down_k = cfg.down_k  # num of nodes for down sampling and fusion with semi-dense points
        self.spot_k = cfg.spot_k  # num of points in a spot
        ...
        for _ in range(self.blocks):
            ...
            self.downsampling.append(Downsampling(cfg.hidden_dim, cfg.hidden_dim))
            if self.geometric_structure_embedding:
                self.self_attentions.append(RPEMambaLayer(
                    cfg.hidden_dim, cfg.num_heads, cfg.dropout, cfg.activation_fn
                ))
            else:
                self.self_attentions.append(PEMambaLayer(
                    cfg.hidden_dim, cfg.num_heads, cfg.dropout, cfg.activation_fn
                ))
            self.cross_attentions.append(TransformerLayer(
                cfg.hidden_dim, cfg.num_heads, cfg.dropout, cfg.activation_fn
